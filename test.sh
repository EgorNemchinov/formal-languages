...
echo "-->Without shared memory"
    ...
    echo "---> Type $type"
    echo "----> Small sample"
    NAME=small_gpu_${type}_no_sm
    python runner.py data/grammar_small.txt data/graph_small.txt --run_time -t $type -o $OUT_DIR/$NAME.txt &&
    python compare_answers.py $OUT_DIR/$NAME.txt data/true_small.txt

    echo "----> Big sample"
    NAME=big_gpu_${type}_no_sm
...
for type in uint8 uint32; do
    echo "---> Type $type"
    echo "----> Small sample"
    NAME=small_gpu_${type}_sm
    python runner.py data/grammar_small.txt data/graph_small.txt --run_time -t $type -s -o $OUT_DIR/$NAME.txt &&
    python compare_answers.py $OUT_DIR/$NAME.txt data/true_small.txt

    echo "----> Big sample"
    NAME=big_gpu_${type}_sm
